...
public class BilateralProjects
{
  ...
  private List<Project> getProjectMates(String employee)
  {
    ...
    List<String> trupleList = new LinkedList<String>();
    ...
    for(String s: map.keySet())
    {
      ...
      if(truple.contains(employee))
      {
        trupleList.add(s);
      }

      if(truple.getName().equals(employee))
      {
        //truple.setCount(0);
        trupleList.add(s);
      }
    }
    map.keySet().removeAll(trupleList);
    return list;
  }
